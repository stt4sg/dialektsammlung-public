      ...
      <div className="info">
        ...
        <div className="numbers">
            ...
            {loading ? (
              ...
              <>
                ...
                <span className="value">
                  <b>{formatProgress(progress)}</b> / {formatProgress(progressTotal)}
                </span>
              </>
            )}
          </div>
          {loading ? (
            ...
            <ProgressBar progress={progress / progressTotal} secondary={progressSecondary} />
          )}
        </div>
      </div>
      {loading ? <button>Loading...</button> : children && <button onClick={onClick}>{children}</button>}
    </li>
  );
}
...
} & ((InProgressLanguage & { type: 'in-progress' }) | (LaunchedLanguage & { type: 'launched' }));
    ...
    <>
      {showModal && (
        <LocalizationProvider l10n={l10n}>
          <GetInvolvedModal locale={locale} onRequestClose={() => setShowModal(false)} />
