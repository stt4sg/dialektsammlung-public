...
const daysBetween = (date1: Date, date2: Date) => Math.floor((date1.getTime() - date2.getTime()) / ONE_DAY);
...
export async function computeGoals(client_id: string): Promise<any> {
  ...
  for (const { locale_id, type, created_at } of rows) {
    ...
    const daysSinceLastActivity = streak.lastActivityAt ? daysBetween(contributionAt, streak.lastActivityAt) : 0;
    if (!streak.startedAt) {
...
export default async function getGoals(client_id: string, locale?: string): Promise<AllGoals['globalGoals']> {
 ...
 */
export async function checkGoalsAfterContribution(client_id: string, locale: { id: number } | { name: string }) {
  if (!(await hasComputedGoals(client_id))) return;
