...
export default class Bucket {
   ...
   */
  getPublicUrl(key: string, bucketType?: string) {
    return this.s3.getSignedUrl('getObject', {
      Bucket: bucketType === 'dataset' ? getConfig().DATASET_BUCKET_NAME : getConfig().CLIP_BUCKET_NAME,
      Key: key,
      Expires: 60 * 60 * 12,
    });
  }
  ...
  async getRandomClips(client_id: string, locale: string, count: number): Promise<Clip[]> {
    ...
    const clips = await this.model.findEligibleClips(client_id, locale, Math.ceil(count * 1.5));
