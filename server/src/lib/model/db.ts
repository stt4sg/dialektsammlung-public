...
import { ChallengeToken, Sentence, taxonomies, TaxonomyToken } from 'common';
...
export const getParticipantSubquery = (condition: 'self' | 'team' | 'general') => {
  return `
...
export async function getLocaleId(locale: string): Promise<number> {
  ...
  if (!localeIds) {
    const [rows] = await getMySQLInstance().query('SELECT id, name FROM locales');
...
export async function getTermIds(term_names: string[]): Promise<number[]> {
  if (!termIds) {
    const [rows] = await getMySQLInstance().query(`SELECT id, term_sentence_source FROM taxonomy_terms`);
    termIds = rows.reduce(
...
export default class DB {
  ...
  /**
   * @see DB.findSentencesWithFewClips
   */
  findSentencesNeedingClips = (client_id: string, count: number) => this.findSentencesWithFewClips(client_id, count);

  /**
   * Ensure the database is setup.
   */
  ensureSetup = async () => this.schema.ensure();

  /**
   * I hope you know what you're doing.
   * @todo this seems to be only used for the test code? and even then it does not seem to work if i look at all my test-voiceweb_xx
   */
  ...
  async getSpeakerCount(locales: string[]): Promise<{ locale: string; count: number }[]> {
    return (
  ...
  /**
   * Fetch random sentences based on:
   * - source importance randomly with weights: 0.8 tamedia_sentences, 0.2 cv_sentences.
   * - make sure the user has not seen it already.
   * - clips_count = 0 OR has_valid_clip = 0 ; single sentences could be changed once every sentence is spoken at once.
   * - clips_count ASC ; ones with no clip first
   */
  async findSentencesWithFewClips(client_id: string, count: number): Promise<Sentence[]> {
    const source = Math.random() < 0.8 ? 'tamedia_sentences' : 'cv_sentences';
    const [rows] = await this.mysql.query(
      `
        SELECT *
        FROM (
               ...
               WHERE is_used
                 ...
                 SELECT *
                 FROM clips
                 WHERE clips.original_sentence_id = sentences.id
                   AND clips.client_id = ?
                 )
                 AND NOT EXISTS(
                 SELECT *
                 FROM skipped_sentences skipped
                 WHERE skipped.sentence_id = sentences.id
                   AND skipped.client_id = ?
                 )
                 AND (clips_count = 0 AND has_valid_clip = 0)
        ...
        LIMIT ?
      `,
      [source, client_id, client_id, SHUFFLE_SIZE, count]
    );
  ...
  async findSentencesMatchingTaxonomy(
    ...
    const [rows] = await this.mysql.query(
      `
        ...
        LIMIT ?
      `,
      [await getTermIds(segments), locale_id, client_id, client_id, SHUFFLE_SIZE, count]
    );

    return (rows || []).map(({ id, text, term_name, term_sentence_source }: any) => ({
      id,
      text,
  ...
  /**
   * Fetch random clips based on:
   * - make sure it's not the user's.
   * - clips_count <= 15 ; not sure this makes sense for our use-case
   * - is_valid IS NULL ; sentences are only validated as long as they are neither good or bad. see {@link DB#saveVote}
   * - ORDER BY clips_count ASC, created_at ASC ; oldest ones first
   */
  async findClipsNeedingValidation(client_id: string, locale: string, count: number): Promise<DBClipWithVoters[]> {
    const locale_id = await getLocaleId(locale);
    return await this.findClipsWithFewVotes(client_id, locale_id, count);
  }

  async findClipsWithFewVotes(client_id: string, locale_id: number, count: number): Promise<DBClipWithVoters[]> {
  ...
  async findClipsMatchingTaxonomy(
    ...
    const [clips] = await this.mysql.query(
      ...
      ORDER BY RAND()
      LIMIT ?`,
      [locale_id, locale_id, client_id, await getTermIds(segments), client_id, client_id, SHUFFLE_SIZE, count]
    );
    for (const clip of clips) {
      clip.voters = clip.voters ? clip.voters.split(',') : [];
      clip.taxonomy = {
   ...
   */
  async createOrVerifyUserClient(id: string, auth_token?: string): Promise<boolean> {
  ...
  /**
   * what does this do:
   * - create or verify the user_client
   * - insert the vote into the database
   * - update updated_clips.is_valid TRUE/FALSE if it's been decided that it's a good/bad(2 up/down votes) clip else NULL
   * - sentences.has_valid_clip if it's been decided that it's a good clip
   */
  ...
  async getValidClipCount(locales: string[]): Promise<{ locale: string; count: number }[]> {
    const [rows] = await this.mysql.query(
      `
  ...
  async getClipsStats(locale?: string): Promise<{ date: string; total: number; valid: number }[]> {
    const localeId = locale ? await getLocaleId(locale) : null;

    const intervals = ['100 YEAR', '12 MONTH', '9 MONTH', '6 MONTH', '3 MONTH', '0 HOUR'];
    const ranges = intervals
      .map(interval => 'NOW() - INTERVAL ' + interval)
      .reduce(
        (ranges, interval, i, intervals) =>
          i + 1 === intervals.length ? ranges : [...ranges, [interval, intervals[i + 1]]],
        []
      );
  ...
  async getVoicesStats(locale?: string): Promise<{ date: string; value: number }[]> {
  ...
  async getContributionStats(locale?: string, client_id?: string): Promise<{ date: string; value: number }[]> {
    // It's necesary to manually create an array of all of the hours, because otherwise
    // if a time interval has no contributions, that hour will just get dropped entirely
    const hours = Array.from({ length: 10 }).map((_, i) => i);
  ...
  async findClip(id: string) {
    return (await this.mysql.query('SELECT * FROM clips WHERE id = ? LIMIT 1', [id]))[0][0];
  }

  async findSentence(id: string) {
    return (await this.mysql.query('SELECT locale_id, text FROM sentences WHERE id = ? LIMIT 1', [id]))[0][0];
  }
  ...
  async getDashboardStatsOne(votes = true, interval: boolean, client_id?: string) {
    return (
      await this.mysql.query(
        `SELECT COUNT(id) AS count
        FROM ${votes ? 'votes' : 'clips'}
        WHERE true ${interval ? 'AND created_at > CURDATE() - INTERVAL 1 DAY' : ''} ${
  ...
  async getDashboardStats(clientId: string) {
    return {
      votesPerDay: await this.getDashboardStatsOne(true, true),
      votesTotal: await this.getDashboardStatsOne(true, false),
      clipPerDay: await this.getDashboardStatsOne(false, true),
      clipTotal: await this.getDashboardStatsOne(false, false),

      votesPerDayClient: await this.getDashboardStatsOne(true, true, clientId),
      votesTotalClient: await this.getDashboardStatsOne(true, false, clientId),
      clipPerDayClient: await this.getDashboardStatsOne(false, true, clientId),
      clipTotalClient: await this.getDashboardStatsOne(false, false, clientId),
    };
  }

  async getCantonStats() {
    const [cantonClipCount] = await this.mysql.query(
      `
        SELECT cantons.canton, cantons.name, cantons.flag, IF(count IS NULL, 0, count) AS count FROM cantons
          LEFT JOIN (
            SELECT canton, COUNT(*) AS count
            FROM clips
              JOIN user_clients ON clips.client_id = user_clients.client_id
            GROUP BY canton ) t ON cantons.canton = t.canton
        ORDER BY cantons.canton;
      `
    );
    return cantonClipCount;
  }

  async getCantonAvgAcceptanceRatio() {
    const [cantonAcceptanceRatio] = await this.mysql.query(
      `
        SELECT cantons.canton, IF(acceptance_ratio IS NULL, 0.5, acceptance_ratio) AS acceptance_ratio, sum_pos, sum_all  FROM
          cantons LEFT JOIN (
            SELECT t1.canton, sum_pos/sum_all AS acceptance_ratio, sum_pos, sum_all
            FROM cantons c
                   LEFT JOIN
                 /* SUM pos votes grouped by canton */
                   (
                     SELECT user_clients.canton AS canton, COUNT(user_clients.canton) AS sum_pos
                     FROM user_clients
                            JOIN clips c ON user_clients.client_id = c.client_id
                            JOIN votes v ON c.id = v.clip_id
                     WHERE v.is_valid
                     GROUP BY user_clients.canton
                   ) t1 ON t1.canton = c.canton
                   LEFT JOIN
                 /* SUM all votes grouped by canton*/
                   (
                     SELECT user_clients.canton AS canton, COUNT(user_clients.canton) AS sum_all
                     FROM user_clients
                            JOIN clips c ON user_clients.client_id = c.client_id
                            JOIN votes v ON c.id = v.clip_id
                     GROUP BY user_clients.canton
                   ) t2 ON t2.canton = c.canton
            GROUP BY canton) s ON cantons.canton = s.canton
        ORDER BY cantons.canton;
      `
    );
    return cantonAcceptanceRatio;
  }

  async getCantonSpeakerFactor() {
    const [cantonSpeakerFactor] = await this.mysql.query(
      `
        SELECT cantons.canton, max/(numberOfPeople * germanSpeakingRate) as swissGermanSpeakersFactor
            FROM
              (SELECT MAX(numberOfPeople * germanSpeakingRate) AS max FROM cantons) t,
               cantons
        ORDER BY cantons.canton;
      `
    );
    return cantonSpeakerFactor;
  }
  ...
  async insertNewsletterEmailForUnregisteredUser(client_id: string, auth_token: string, email: string) {
    await this.mysql.query(
      `
        UPDATE user_clients
        SET email = ?, wants_newsletter = ?
        WHERE client_id = ? AND auth_token = ?
      `,
      [email, 1, client_id, auth_token]
    );
  }

  async updateDialectUnregisteredUser(client_id: string, auth_token: string, zipcode: string, canton: string) {
    await this.mysql.query(
      `
        UPDATE user_clients
        SET zipcode = ?, canton = ?
        WHERE client_id = ? AND auth_token = ?
      `,
      [zipcode, canton, client_id, auth_token]
    );
  }

  async createReport(
    client_id: string,
    { kind, id, reasons }: { kind: 'clip' | 'sentence'; id: string; reasons: string[] }
  ) {
    const [table, column] = kind == 'clip' ? ['reported_clips', 'clip_id'] : ['reported_sentences', 'sentence_id'];
    for (const reason of reasons) {
      await this.mysql.query(`INSERT INTO ${table} (client_id, ${column}, reason) VALUES (?, ?, ?)`, [
        client_id,
        id,
        reason,
      ]);
  ...
  async deleteClip(id: string) {
    await this.mysql.query(`DELETE FROM clip_demographics WHERE clip_id = ?`, [id]);
  ...
  private getPrioritySegments(locale: string): string[] {
    return Object.keys(taxonomies)
      .filter((taxonomyToken: TaxonomyToken) => taxonomies[taxonomyToken].locales.includes(locale))
