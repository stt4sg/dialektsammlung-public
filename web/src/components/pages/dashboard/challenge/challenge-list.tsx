...
const PointsIcon = ({ className }: { className: string }) => <div className={`star-points ${className}`} />;
  ...
  render() {
    ...
    const items = rows.map((row, i) => {
      ...
      return [
        ...
        team ? (
          <li key={row.position} className={`row team${isYou ? ' you' : ''}`} ref={isYou ? this.youRow : null}>
          ...
          <li key={row.position} className={`row${isYou ? ' you' : ''}`} ref={isYou ? this.youRow : null}>
            <div className="ranking">
              <div className="position">
                {row.position + 1 < 10 && '0'}
                {row.position + 1}
