...
function* asBundleGenerator(localeMessages: string[][], messageOverwrites?: MessageOverwrites) {
  for (const [locale, messages] of localeMessages) {
    const bundle = new FluentBundle(locale, { useIsolating: false });
    bundle.addResource(
      new FluentResource(messages + (messageOverwrites?.[locale] ? '\n' + messageOverwrites[locale] : ''))
    );
    yield bundle;
  }
}

export function createCrossLocalization(localeMessages: string[][], locales: string[]) {
  const currentLocales = negotiateLocales([...locales, ...navigator.languages]);

  localeMessages = localeMessages
    .filter(([locale]) => currentLocales.includes(locale))
    .sort(([locale1], [locale2]) => (currentLocales.indexOf(locale1) > currentLocales.indexOf(locale2) ? 1 : -1));

  return new ReactLocalization(asBundleGenerator(localeMessages));
}

export async function createLocalization(api: API, userLocales: string[], messageOverwrites: MessageOverwrites) {
