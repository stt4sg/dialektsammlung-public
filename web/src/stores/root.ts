...
const store = createStore(
  function root(
    { sentences, user, clips, flags, requestedLanguages, locale, notifications, uploads, rank }: StateTree = {
      api: undefined,
      clips: undefined,
      flags: undefined,
      locale: undefined,
    ...
    const newState = {
      ...
      requestedLanguages: RequestedLanguages.reducer(requestedLanguages, action as RequestedLanguages.Action),
...
function loadModalSetting(email: string) {
  return function () {
    fetch('/api/v1/modal-settings?email=' + email)
      .then(function (resp) {
        return resp.json();
      })
      .then(function (json) {
        let settings = json.modal_settings[0];
        store.dispatch(User.actions.saveSettings(settings));
      });
  };
}
...
store.dispatch(User.actions.saveSettings(null));
...
(async function () {
  ...
  const { locale, api } = globalState;
  const { leaderboard, my_rank: myRank } = await api.forLocale(locale).fetchLeaderboard('clip');
  store.dispatch({ type: 'SET_RANK', rank: myRank });
})();

let isFetchingUserSettings = false;

store.subscribe(async () => {
  const { locale, user, api } = store.getState();

  if (user && user.account && user.account.email && isFetchingUserSettings === false) {
    isFetchingUserSettings = true;
    store.dispatch(loadModalSetting(user.account.email));
  }

  if (typeof ga === 'function' && (!prevUser || !prevUser.account) && user.account) {
