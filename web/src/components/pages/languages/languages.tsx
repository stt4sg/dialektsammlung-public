...
//TODO most of this file is not used any more but it probably does not enable data-leakage
...
class LanguagesPage extends React.PureComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    const localeMessages = await api.fetchCrossLocaleMessages();
  ...
  sortLocales = () => {
    ...
    function presortLanguages<T extends BaseLanguage>(sortFn: (l1: T, l2: T) => number): (l1: T, l2: T) => number {
      return (l1, l2) => {
    ...
    inProgress.sort(
      presortLanguages((l1, l2) =>
        l1.sentencesCount < l2.sentencesCount || l1.localizedPercentage < l2.localizedPercentage ? 1 : -1
      )
    );
  ...
  handleQueryChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    this.setState({
      ...
      query,
      selectedSection:
        filteredInProgress.length == 0 ? 'launched' : filteredLaunched.length == 0 ? 'in-progress' : selectedSection,
    });
  };
  ...
  render() {
    ...
    const inProgressCountLabel = query ? <span className="count">({filteredInProgress.length})</span> : '';
      ...
      <div className={'selected-' + selectedSection}>
        ...
        <div className="top">
          ...
          <div className="text">
            <div className="inner">
              <h2>
                <Localized id="request-language-text" /> <Localized id="request-language-text-learn" />
              </h2>
        ...
        <div className="mobile-headings">
          ...
          <div className="labels">
            <h2 className="launched" onClick={this.changeSection.bind(this, 'launched')}>
              {getString('language-section-launched')}
              {launchedCountLabel}
            </h2>

            <h2 className="in-progress" onClick={this.changeSection.bind(this, 'in-progress')}>
        ...
        <div className="language-sections">
          <section className="launched">
            ...
            <ul>
              {launched.length > 0
                ? (query || showAllLaunched ? filteredLaunched : filteredLaunched.slice(0, 3)).map(
                    (localization, i) => (
                      <LocalizationBox
          ...
          <section className="in-progress">
            ...
            <ul>
              {inProgress.length > 0
                ? (query || showAllInProgress ? filteredInProgress : filteredInProgress.slice(0, 3)).map(
                    (localization, i) => (
                      <LocalizationBox
                        key={localization.locale}
                        localeMessages={localeMessages}
...
export default connect<PropsFromState>(mapStateToProps)(withLocalization(LanguagesPage));
