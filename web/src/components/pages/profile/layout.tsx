...
import { BarChartIcon, CameraIcon, CloudIcon, CogIcon, TrashIcon, UserIcon, UserPlusIcon } from '../../ui/icons';
...
function downloadData(account: UserClient) {
  const text = [
    ...Object.entries(pick(account, 'email', 'username', 'age', 'gender', 'zipcode', 'canton')),
    ...account.locales.reduce((all, l, i) => {
      const localeLabel = 'language ' + (i + 1);
      return [...all, [localeLabel, l.locale], [localeLabel + ' accent', l.accent]];
    }, []),
  ]
    ...
    <div className="profile-layout">
      <div className="profile-nav">
        <div className="links">
          {[
            ...
            //TODO part of https://github.com/stt4sg/STT4SG/issues/29
      ...
      <div className="content">
        <Switch>
          ...
          ].map(({ route, Component }: { route: string; Component: any }) => (
            <Route
              key={route}
              exact
              path={route}
          ...
          <Route render={() => <Redirect to={toLocaleRoute(URLS.PROFILE_INFO)} />} />
        </Switch>
      </div>
    </div>
  );
...
export default connect<PropsFromState>(({ user }: StateTree) => ({ user }))(localeConnector(Layout));
