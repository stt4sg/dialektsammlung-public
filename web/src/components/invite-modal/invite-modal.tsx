...
export default ({ enrollment: { challenge, team, invite }, ...props }: InviteModalProps) => {
  ...
  useEffect(() => {
    if (copiedRecently) {
      ...
      api
        .fetchInviteStatus()
        .then(({ showInviteSendToast = false, hasEarnedSessionToast = false, challengeEnded = true }) => {
          sessionStorage.setItem('showInviteSendToast', JSON.stringify(showInviteSendToast));
          sessionStorage.setItem('hasEarnedSessionToast', JSON.stringify(hasEarnedSessionToast));
