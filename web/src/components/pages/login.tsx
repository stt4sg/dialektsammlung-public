...
export const LoginFailure = withRouter(({ history }: RouteComponentProps<any, any, any>) => {
...
export const LoginSuccess = withRouter(({ history, location }: RouteComponentProps<any, any, any>) => {
  ...
  useEffect(() => {
    ...
    history.replace(
      account ? redirectURL || toLocaleRoute(URLS.DASHBOARD + location.search) : URLS.PROFILE_INFO + location.search
    );
  }, [user]);
