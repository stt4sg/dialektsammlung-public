...
export namespace Sentences {
  ...
  const localeSentences = ({ locale, sentences }: StateTree) => sentences[locale];
  ...
  export const actions = {
    refill: () => async (dispatch: Dispatch<RefillAction>, getState: () => StateTree) => {
      try {
        const state = getState();
        if (Object.keys(localeSentences(state).sentences).length >= MIN_CACHE_COUNT) {
          return;
        }
        const newSentences = await state.api.fetchRandomSentences(CACHE_SET_COUNT);
        dispatch({
          type: ActionType.REFILL,
    ...
    remove:
      (sentenceIds: string[]) => async (dispatch: Dispatch<RemoveAction | RefillAction>, getState: () => StateTree) => {
  ...
  export function reducer(
    ...
    switch (action.type) {
      case ActionType.REFILL:
        const sentenceIds = localeState.sentences.map(s => s.id).concat(localeState.sentences.map(s => s.id));
        return {
          ...state,
          [locale]: {
            sentences: localeState.sentences.concat(action.sentences.filter(({ id }) => !sentenceIds.includes(id))),
            isLoading: false,
          },
        };
