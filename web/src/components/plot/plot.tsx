...
> {
  ...
  render() {
    let { children, data, formatNumber, max, renderXTickLabel, tickCount, tickMultipliers, ...props } = this.props;
    ...
    return (
      <svg width="100%" height="100%" className="plot" {...props} ref={this.svgRef}>
        {Array.from({ length: tickCount }).map((_, i) => {
          const y = (i * TOTAL_LINE_MARGIN) / tickCount + Y_OFFSET;
          return (
            <React.Fragment key={i}>
              <text className="tick-label" x={TEXT_OFFSET} y={y} dominantBaseline="middle" textAnchor="end">
                {formatNumber(Math.round(((tickCount - 1 - i) * max) / (tickCount - 1)))}
              </text>
              <line x1={LINE_OFFSET} y1={y} x2={width + PLOT_PADDING} y2={y} stroke="rgba(0,0,0,0.2)" />
            </React.Fragment>
          );
        })}
        {data.map((datum, i) => (
          <text
            ...
            x={LINE_OFFSET + i * ((width - PLOT_PADDING - TEXT_OFFSET) / data.length)}
...
export const BarPlot = ({ data }: { data: { date: string; value: number }[] }) => (
  <Plot
    data={data}
    ...
    {({ max, width }) => {
      ...
      const getBarX = (i: number) =>
        LINE_OFFSET + PLOT_PADDING - barWidth / 2 + (i * (width - PLOT_PADDING - TEXT_OFFSET)) / BAR_COUNT;

      return (
        <>
          {Array.from({ length: BAR_COUNT }).map((_, i) => (
            <rect key={i} className="bg" x={getBarX(i)} y={Y_OFFSET} width={barWidth} height={BAR_HEIGHT} />
          ))}
