...
const getChallenge = (user: User.State): string => {
  return user?.account?.enrollment?.challenge ? user.account.enrollment.challenge : null;
};
...
export default class API {
  ...
  private async fetch(path: string, options: FetchOptions = {}): Promise<any> {
    ...
    if (path.startsWith(location.origin) && !user.account && user.userId) {
      finalHeaders['Authorization'] = 'Basic ' + btoa(user.userId + ':' + user.authToken);
    }
  ...
  getDashboardStats = () => this.fetch(`${API_PATH}/getDashboardStats`);

  getCantonStats = () => this.fetch(`${API_PATH}/getCantonStats`);
  ...
  async fetchCrossLocaleMessages(): Promise<string[][]> {
    return Object.entries(await this.fetch(`/cross-locale-messages.json`)) as string[][];
  }
  ...
  fetchDocument(name: 'privacy' | 'terms' | 'challenge-terms' | 'pr-coverage'): Promise<string> {
    return this.fetch(`/${name}.html`, { isJSON: false });
  }
  ...
  > {
    return this.fetch(API_PATH + (locale ? '/' + locale : '') + '/contribution_activity?from=' + from);
  }
  ...
  saveUnregisteredAccount(body: any): Promise<void> {
    return this.fetch(API_PATH + '/unregistered_dialect', {
      method: 'PATCH',
      body,
    });
  }

  deleteAccount(data: DeleteUserClient): Promise<void> {
    return this.fetch(API_PATH + '/user_client', {
      method: 'DELETE',
      body: data,
    });
  ...
  subscribeToNewsletter(body: any): Promise<void> {
    return this.fetch(API_PATH + '/newsletter', {
      method: 'POST',
      body,
    });
  }
  ...
  getRank = () => this.fetch(this.getClipPath() + '/rank');
  ...
  fetchGoals(locale?: string): Promise<AllGoals> {
    return this.fetch(API_PATH + '/user_client' + (locale ? '/' + locale : '') + '/goals');
  }

  claimAccount(): Promise<void> {
    return this.fetch(API_PATH + '/user_clients/' + this.user.userId + '/claim', { method: 'POST' });
  }

  seenAwards(kind: 'award' | 'notification' = 'award'): Promise<void> {
    return this.fetch(API_PATH + '/user_client/awards/seen' + (kind == 'notification' ? '?notification' : ''), {
      method: 'POST',
    });
  }
  ...
  fetchChallengePoints(): Promise<{
    ...
    if (getChallenge(this.user)) {
      return this.fetch(`${API_PATH}/challenge/${this.user.account.enrollment.challenge}/points`);
    }
    return null;
  }
  ...
  fetchWeeklyProgress(): Promise<WeeklyChallenge> {
    if (getChallenge(this.user)) {
      return this.fetch(`${API_PATH}/challenge/${this.user.account.enrollment.challenge}/progress`);
    }
    return null;
  }

  fetchTopTeams(locale?: string, cursor?: [number, number]): Promise<TeamChallenge[]> {
    if (getChallenge(this.user)) {
      return this.fetch(
        `${API_PATH}/challenge/${this.user.account.enrollment.challenge}/${locale}/teams?cursor=${
  ...
  fetchTopContributors(locale?: string, type?: 'vote' | 'clip', cursor?: [number, number]): Promise<Challenge[]> {
    if (getChallenge(this.user)) {
      return this.fetch(
        `${API_PATH}/challenge/${this.user.account.enrollment.challenge}/${locale}/contributors/${type}?cursor=${
          cursor ? JSON.stringify(cursor) : ''
        }`
      );
    }
  ...
  fetchTopMembers(locale?: string, type?: 'vote' | 'clip', cursor?: [number, number]): Promise<Challenge[]> {
    if (getChallenge(this.user)) {
      return this.fetch(
        `${API_PATH}/challenge/${this.user.account.enrollment.challenge}/${locale}/members/${type}?cursor=${
          cursor ? JSON.stringify(cursor) : ''
        }`
      );
    }
  ...
  fetchInviteStatus(): Promise<{
    ...
    if (getChallenge(this.user)) {
      return this.fetch(`${API_PATH}/challenge/${this.user.account.enrollment.challenge}/achievement/invite`);
    }
    return null;
  }
  ...
  setInviteContributeAchievement(): Promise<void> {
    if (getChallenge(this.user)) {
      return this.fetch(`${API_PATH}/challenge/${this.user.account.enrollment.challenge}/achievement/session`, {
        method: 'POST',
      });
    }
    return null;
  ...
  getPublicUrl(path: string, bucketType: string, useCDN: boolean): Promise<{ url: string }> {
