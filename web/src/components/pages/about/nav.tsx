...
import { FlagIcon, HeartIcon, LayersIcon } from '../../ui/icons';
import './nav.css';
...
const MENU_SECTIONS: any[] = [
  [SECTIONS.WHY_COMMON_VOICE, ['menu1-active.png', 'menu1-inactive.png'], FlagIcon, { className: 'i1' }],
  [SECTIONS.HOW_IT_WORKS, ['menu2-active.png', 'menu2-inactive.png'], LayersIcon, { className: 'i2' }],
  // [
  //   SECTIONS.PARTNERS,
  ...
  //   { className: 'i3' },
  // ],
  [SECTIONS.GET_INVOLVED, ['menu4-active.png', 'menu4-inactive.png'], HeartIcon, { className: 'i4' }],
];
...
const Nav: React.ComponentType<Props> = React.memo((props: Props) => {
  ...
  const parentProps: any = {
    className: classnames('nav', props.navType),
  };
  ...
  return (
    <div {...parentProps}>
      {MENU_SECTIONS.map(([key, [activeBg, inactiveBg], MobileIcon, { className, ...otherProps }]) => (
        <a
          key={key}
          ...
          {props.navType === 'mobile' && (
            <>
              <img src={require('./images/nav/nav-mobile.png')} className="mobile-rounded" />
