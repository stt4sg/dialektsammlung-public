...
class RequestLanguageModal extends React.Component<Props, State> {
  state: State = {
    ...
    isSubmitted: false,
    language: NATIVE_NAMES[navigator.languages.find(lang => lang.split('-')[0] !== 'en')],
    otherLanguage: '',
        ...
        {isSubmitted ? (
          ...
          <form onSubmit={this.save}>
            ...
            <Localized id="email-input" attrs={{ label: true }}>
              <LabeledInput label="Email" name="email" required type="email" value={email} onChange={this.update} />
            </Localized>

            <label className="opt-in">
              <input name="sendEmails" type="checkbox" checked={sendEmails} onChange={this.update} />
            ...
            <div className="actions">
              <Localized id="submit-form-action">
                <Button disabled={!email || (language == 'other' ? !otherLanguage : !language)} type="submit" rounded />
              </Localized>
              <div />
            </div>
...
export default connect<PropsFromState, any>(mapStateToProps, mapDispatchToProps)(RequestLanguageModal) as any;
