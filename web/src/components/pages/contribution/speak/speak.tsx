...
import { getTrackClass, trackRecording } from '../../../../services/tracker';
import URLS from '../../../../urls';
import { localeConnector, LocalePropsFromState } from '../../../locale-helpers';
import Modal, { ModalButtons } from '../../../modal/modal';
import TermsModal from '../../../terms-modal';
import { ArrowRight, CheckIcon, ChromeColor, FirefoxColor, MicIcon, SafariColor, StopIcon } from '../../../ui/icons';
...
import { RecordButton, RecordingStatus } from '../../../primary-buttons/primary-buttons';
...
import SpeakPageModal from './../../../../components/speak-page-modal/SpeakPageModal';
...
const NoSentencesAvailable = () => (
  <div className="empty-container">
    <div className="error-card card-dimensions no-sentences-available">
      ...
      <LinkButton rounded blank href="https://common-voice.github.io/sentence-collector/">
...
class SpeakPage extends React.Component<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props, state: State) {
    if (state.clips.length > 0) {
      const sentenceIds = state.clips.map(({ sentence }) => (sentence ? sentence.id : null)).filter(Boolean);
      const unusedSentences = props.sentences.filter(s => !sentenceIds.includes(s.id));
      return {
        clips: state.clips.map(clip =>
          clip.sentence ? clip : { recording: null, sentence: unusedSentences.shift() || null }
        ),
      };
    }
    ...
    if (props.sentences.length > 0) {
      return {
        clips: props.sentences.slice(0, SET_COUNT).map(sentence => ({ recording: null, sentence })),
      };
    }
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    if (prevState.isSpeakModalVisible == false) {
      this.showSpeakModalIfApplicable();
    }
  }
  ...
  componentDidMount() {
    ...
    if (!this.audio.isMicrophoneSupported() || !this.audio.isAudioRecordingSupported()) {
      this.isUnsupportedPlatform = true;
    }
    this.showSpeakModalIfApplicable();
  }

  showSpeakModalIfApplicable() {
    const user = this.props.user;
    if (user === null || (user && user.account === null)) {
      const cookies = new Cookies();
      if (cookies.get('seen_speak_modal') !== '1') {
        this.setState({
          ...this.state,
          isSpeakModalVisible: true,
        });
        cookies.set('seen_speak_modal', 1, { path: '/' });
        document.cookie = document.cookie + ';seen_speak_modal=1;';
      } else if (cookies.get('seen_speak_modal') === '1') {
        // do nothing
      }
    } else if (user !== null && user.account !== null && user.settings) {
      const cookies = new Cookies();
      if (user.settings.seen_speak_modal === 0) {
        this.setState({
          ...this.state,
          isSpeakModalVisible: true,
        });
        this.markSpeakModalAsSeen(user);
      } else if (user.settings.seen_speak_modal === 1) {
        // do nothing
      }
    }
  }

  markSpeakModalAsSeen(user: any) {
    let email = user.account.email;
    let authToken = user.authToken;
    const modalType = 'speak-modal';
    const self = this;
  ...
  private getRecordingIndex() {
    return this.state.rerecordIndex ?? this.state.clips.findIndex(({ recording }) => !recording);
  }
  ...
  private getRecordingError = (): RecordingError => {
    ...
    const minClipLength = currentSentence.taxonomy ? MIN_RECORDING_MS_BENCHMARK : MIN_RECORDING_MS;
        ...
        while (retries) {
          try {
            ...
            if (error.message === 'save_clip_error') {
              msg = 'Upload of this clip keeps failing at server, reload the page or try after sometime';
            } else {
              msg = 'Upload of this clip keeps failing, keep retrying?';
            }
            retries--;
            await new Promise(resolve => setTimeout(resolve, 1000));
            if (!DEV && retries == 0 && confirm(msg)) {
              retries = 3;
            }
          }
        }
  ...
  private resetState = (callback?: any) => this.setState(initialState, callback);
  ...
  private displayError = () => {
    return this.isUnsupportedPlatform || (!this.props.isLoading && this.state.clips.length == 0);
  };

  private returnSpeakError = () => {
    return this.isUnsupportedPlatform ? <UnsupportedInfo /> : <NoSentencesAvailable />;
  };
  ...
  render() {
    ...
    return (
      <>
        <div id="speak-page">
          <NavigationPrompt when={clips.filter(clip => clip.recording).length > 0}>
            {({ onConfirm, onCancel }: any) => (
              <Modal innerClassName="record-abort" onRequestClose={onCancel}>
                ...
                <Localized id="record-abort-delete">
                  <TextButton className={getTrackClass('fs', 'exit-delete-clips')} onClick={onConfirm} />
                </Localized>
              </Modal>
            )}
          </NavigationPrompt>
          {showPrivacyModal && <TermsModal onAgree={this.agreeToTerms} onDisagree={this.toggleDiscardModal} />}
          {isSpeakModalVisible && <SpeakPageModal onCancel={this.onCancel}></SpeakPageModal>}
          {showDiscardModal && (
          ...
          <ContributionPage
                ...
                <Localized
                  id={
                    this.isRecording
                      ...
                      : ['record-instruction', 'record-again-instruction'][recordingIndex] ||
            ...
            onSubmit={() => {
              if (!localStorage.getItem(ACTIVE_INDEX)) {
                localStorage.setItem(ACTIVE_INDEX, JSON.stringify(5));
              } else {
                localStorage.setItem(ACTIVE_INDEX, JSON.parse(localStorage.getItem(ACTIVE_INDEX)) + 5);
              }
              this.upload();
            }}
            primaryButtons={
              <RecordButton trackClass="speak-record" status={recordingStatus} onClick={this.handleRecordClick} />
            }
            pills={clips.map((clip, i) => (props: ContributionPillProps) => (
              <RecordingPill
                {...props}
                clip={clip}
                status={recordingIndex === i ? 'active' : clip.recording ? 'done' : 'pending'}
            ...
            reportModalProps={{
              reasons: ['offensive-language', 'grammar-or-spelling', 'different-language', 'difficult-pronounce'],
              kind: 'sentence',
              id: recordingIndex == -1 || !clips[recordingIndex].sentence ? null : clips[recordingIndex].sentence.id,
            }}
...
export default withRouter(
  localeConnector(withLocalization(connect<PropsFromState, any>(mapStateToProps, mapDispatchToProps)(SpeakPage)))
);
