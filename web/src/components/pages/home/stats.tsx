...
import Plot, { BarPlot, LINE_OFFSET, PLOT_PADDING, TOTAL_LINE_MARGIN, Y_OFFSET } from '../../plot/plot';
...
export namespace ClipsStats {
  ...
  const Metric = ({ data, labelId, attribute }: { data: any[]; labelId: string; attribute: Attribute }) => (
  ...
  const Path = React.forwardRef(
    (
      ...
      const pointFromDatum = (x: number, y: number): [number, number] => [
        LINE_OFFSET + PLOT_PADDING + (x * (width - LINE_OFFSET - 2 * PLOT_PADDING - CIRCLE_RADIUS)) / (data.length - 1),
        Y_OFFSET - PLOT_STROKE_WIDTH / 2 + ((1 - y / max) * (data.length + 1) * TOTAL_LINE_MARGIN) / TICK_COUNT,
      ];
      ...
      return (
        <>
          <path
            d={spline.svgPath(spline.points(data.map((datum: any, i: number) => pointFromDatum(i, datum[attribute]))))}
            className={attribute}
            fill="none"
            ref={ref}
            strokeWidth={PLOT_STROKE_WIDTH}
          />
          <circle cx={x} cy={y} r={CIRCLE_RADIUS} fill="white" className={'outer ' + attribute} />
        ...
        <StatsCard
          ...
          <Tooltip
            ...
            <Plot
              ...
              max={data.reduce((max: number, d: any) => Math.max(max, d.total, d.valid), 0)}
              onMouseMove={this.handleMouseMove}
              onMouseOut={this.handleMouseOut}
              renderXTickLabel={({ date }: any) => {
                const dateObj = new Date(date);
                const dayDiff = Math.ceil(Math.abs(dateObj.getTime() - new Date().getTime()) / (1000 * 3600 * 24));
          ...
          header={
            <div>
              ...
              <div className="online-voices">
                {data.length > 0 ? data[data.length - 1].value.toLocaleString() : '?'}
              </div>
            </div>
          }
          onLocaleChange={this.updateData}>
