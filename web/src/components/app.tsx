...
import { isMobileSafari, replacePathLocale } from '../utility';
import { createLocalization, DEFAULT_LOCALE, LOCALES } from '../services/localization';
...
import { isContributable, localeConnector, LocalePropsFromState } from './locale-helpers';
import { Flags } from '../stores/flags';
import { LocalizationProvider, ReactLocalization } from '@fluent/react';

const rtlLocales = require('../../../locales/rtl.json');
const ListenPage = React.lazy(() => import('./pages/contribution/listen/listen'));
...
let LocalizedPage: any = class extends React.Component<LocalizedPagesProps, LocalizedPagesState> {
  seenAwardIds: number[] = [];
  ...
  async UNSAFE_componentWillReceiveProps(nextProps: LocalizedPagesProps) {
    ...
    window.onbeforeunload =
      uploads.length > 0
        ? (e: any) => (e.returnValue = 'Leaving the page now aborts pending uploads. Are you sure?')
        : undefined;
    ...
    const award = account?.awards
      ? account.awards.find(a => !a.notification_seen_at && !this.seenAwardIds.includes(a.id))
      : null;

    if (award) {
      this.seenAwardIds.push(...account.awards.map(a => a.id));
      addNotification(`Success, ${award.amount} Clip ${award.days_interval == 1 ? 'daily' : 'weekly'} goal achieved!`, {
        links: [
          {
  ...
  async prepareBundleGenerator({ api, history, userLocales }: LocalizedPagesProps) {
    ...
    documentElement.setAttribute('dir', rtlLocales.includes(mainLocale) ? 'rtl' : 'ltr');

    this.setState({
      l10n: await createLocalization(api, userLocales, this.props.messageOverwrites),
    });
  }
            ...
            <Switch>
              {[
                ...
                <Route
                  ...
                  render={props =>
                    isContributable(locale) ? <Component {...props} /> : <Redirect to={toLocaleRoute(URLS.ROOT)} />
                  }
                />
              ))}
              <Layout />
...
class App extends React.Component {
  ...
  constructor(props: any, context: any) {
    ...
    /* We could query the browser for supported locales here,
     * but currently we only support the default lang (German)
     * for the interface.
     */
    this.userLocales = [DEFAULT_LOCALE];
  }
            ...
            <Switch>
              {Object.values(URLS).map(url => (
                <Route
                  ...
                  exact
                  path={url || '/'}
                  render={() => <Redirect to={`/${userLocale}${url}${location.search}`} />}
                />
              ))}
              <Route
                path="/pt-BR"
                render={({ location }) => <Redirect to={location.pathname.replace('pt-BR', 'pt')} />}
              />
              <Route
                path="/:locale"
                render={({
