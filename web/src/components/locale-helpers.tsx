...
export const LocaleLink = localeConnector(
  ({ blank, dispatch, locale, to, toLocaleRoute, ...props }: { blank: boolean } & LinkProps & LocaleProps) => {
...
export const LocaleNavLink = localeConnector(
  ({ dispatch, locale, to, toLocaleRoute, ...props }: NavLinkProps & LocaleProps) => (
    <NavLink to={toLocaleRoute(to)} {...props} />
  )
);

/**
 * @deprecated remove this? e.g. we only support german anyway?
 */
...
export const ContributableLocaleLock = localeConnector(
  ({
    ...
    return render ? render({ isContributable: isContributable(locale) }) : isContributable(locale) && children;
  }
);
