...
async function updateLocales(clientId: string, locales: { locale: string; accent: string }[]) {
  const [savedLocales]: [{ id: number; locale: string; accent: string }[]] = await db.query(
    `
  ...
  let startAt = savedLocales.findIndex((savedLocale, i) => {
    ...
    return !locale || savedLocale.locale !== locale.locale || savedLocale.accent !== locale.accent;
  });
  if (startAt == -1) {
  ...
  if (deleteIds.length > 0) {
    await db.query('DELETE FROM user_client_accents WHERE id IN (?)', [deleteIds]);
  }

  const newAccents = await Promise.all(
    locales.slice(startAt).map(async l => [clientId, await getLocaleId(l.locale), l.accent])
  );
...
async function updateDemographics(clientId: string, age: string, gender: string) {
...
const UserClient = {
  async findAllWithLocales({ client_id = null, email = null }: { client_id: string; email: string }) {
    const [rows] = await db.query(
      `
        ...
        CROSS JOIN
          (SELECT demographics.age_id, demographics.gender_id
            ...
            WHERE user_clients.${client_id ? `client_id = ?` : `email =?`}
      ...
      `,
      [client_id ? client_id : email, client_id, email]
    );
    return Object.values(
      rows.reduce((obj: { [client_id: string]: any }, row: any) => {
        ...
        obj[row.client_id] = {
          ...
          locales: (client ? client.locales : []).concat(row.accent ? { accent: row.accent, locale: row.locale } : []),
        };
        return obj;
      }, {})
    );
          ...
          --       grouping.
             ...
             (SELECT demographics.age_id, demographics.gender_id
              ...
              ORDER BY updated_at DESC LIMIT 1
               ...
               LEFT JOIN genders ON d.gender_id = genders.id
               LEFT JOIN (
          SELECT enroll.client_id,
                 enroll.url_token     as invite,
                 teams.url_token      AS team,
                 challenges.url_token AS challenge
          FROM enroll
      ...
      : rows.reduce(
          (client: UserClientType, row: any) => ({
            ...
            locales: client.locales.concat(
              typeof row.accent == 'string' ? { accent: row.accent, locale: row.locale } : []
            ),
            awards,
            custom_goals,
            enrollment: {
  ...
  async saveAccount(email: string, { client_id, locales, ...data }: UserClientType): Promise<UserClientType> {
    let [accountClientId, [clients]] = await Promise.all([
      UserClient.findClientId(email),
      email ? db.query('SELECT client_id FROM user_clients WHERE email = ? AND !has_login', [email]) : [],
    ]);
    ...
    const userData = await Promise.all(
      Object.entries({
        has_login: true,
        email,
        ...pick(data, 'username', 'skip_submission_feedback', 'visible', 'canton', 'zipcode', 'wants_newsletter'),
    ...
    if (
      ...
      await earnBonus('sign_up_first_three_days', [data.enrollment.challenge, client_id]);
  ...
  async deleteAccount(email: string, userId: string, { client_id, locales, ...data }: UserClientType): Promise<void> {
    let [accountClientId, [clients]] = await Promise.all([
      UserClient.findClientId(email),
      email ? db.query('SELECT client_id FROM user_clients WHERE email = ? AND !has_login', [email]) : [],
    ]);
    ...
    await db.query(
      `
        DELETE cd
        FROM clips c
        INNER JOIN clip_demographics cd
        ON c.id = cd.clip_id
        WHERE c.client_id = ?;
      `,
      [accountClientId]
    ...
    const ac = new ManagementClient({
      domain: getConfig().AUTH0_API_V2.DOMAIN,
      clientSecret: getConfig().AUTH0_API_V2.CLIENT_SECRET,
      clientId: getConfig().AUTH0_API_V2.CLIENT_ID,
    });
    ac.deleteUser({ id: userId }, function (err) {
      if (err) {
        console.error(err);
      }
    ...
    if (client_id) {
      await db.query(
        `
          ...
          UPDATE basket_token = basket_token`,
  ...
  async updateSSO(old_email: string, email: string): Promise<boolean> {
    ...
    await db.query('UPDATE user_client_newsletter_prefs SET email = ? WHERE email = ?', [email, old_email]);
  ...
  async enrollRegisteredUser(
    ...
    if (email && challengeTokens.includes(challenge) && challengeTeamTokens.includes(team)) {
      ...
      const proceed =
        ...
        registeredUser.every(
          (u: { client_id: string; enroll_id: number; challenge_id: number }) =>
            u.client_id != null && (u.enroll_id == null || u.challenge_id == null)
        );
      ...
      if (proceed) {
        ...
        const [[{ team_id, challenge_id, enrollment_token }]] = await db.query(
          `SELECT t.id AS team_id, t.challenge_id, UUID() AS enrollment_token
           FROM teams t
        ...
        const res = await db.query(
          `
            INSERT INTO enroll (challenge_id, team_id, client_id, url_token, invited_by, referer)
            VALUES (?, ?, ?, ?, ?, ?)
          `,
          [challenge_id, team_id, registeredUser[0].client_id, enrollment_token, invite || null, referer || null]
        );
  ...
  async updateAvatarURL(email: string, url: string) {
    await db.query('UPDATE user_clients SET avatar_url = ? WHERE email = ?', [url, email]);
  },
  ...
  async deleteAvatarClipURL(email: string) {
    await db.query('UPDATE user_clients SET avatar_clip_url = NULL WHERE email = ?', [email]);
  },
  ...
  async claimContributions(to: string, from: string[]) {
    await Promise.all([
      db.query('UPDATE IGNORE clips SET client_id = ? WHERE client_id IN (?)', [to, from]),
      db.query('UPDATE IGNORE votes SET client_id = ? WHERE client_id IN (?)', [to, from]),
    ]);
  },

  async haveSeenSpeakModal(email: string) {
    return new Promise(async function (resolve, reject) {
      await db.query('UPDATE user_clients set seen_speak_modal = 1 where email = ?', [email]);
      resolve(true);
    });
  },

  async haveSeenListenModal(email: string) {
    return new Promise(async function (resolve, reject) {
      await db.query('UPDATE user_clients set seen_listen_modal = 1 where email = ?', [email]);
      resolve(true);
    });
  },

  async fetchModalSettings(email: string) {
    return new Promise(async function (resolve, reject) {
      const [modalStateData] = await db.query(
        'select seen_speak_modal, seen_listen_modal from user_clients where email = ?',
        [email]
      );
      resolve(modalStateData);
    });
  },
};
