...
export default {
  async checkProgress(client_id: string, locale: { id: number } | { name: string }) {
    ...
    const intervalStart = new Date(goal.current_interval_start).toISOString().slice(0, 19).replace('T', ' ');
  ...
  async find(client_id: string) {
    ...
    return rows.map((row: any) =>
      pick(row, 'locale', 'notification_seen_at', 'seen_at', 'type', 'days_interval', 'amount')
    );
  },
