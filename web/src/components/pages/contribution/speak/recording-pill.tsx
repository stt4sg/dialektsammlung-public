...
import { PlayOutlineIcon, RedoIcon, ShareIcon, StopIcon, MicIcon, BlackMicIcon } from '../../../ui/icons';
...
function RecordingPill({ children, clip, getString, onRerecord, onShare, status, ...props }: Props) {
    ...
    <Pill {...props} className="recording" openable status={status}>
      ...
      {!children && status === 'done' && (
        <>
          ...
          <Tooltip arrow open={isPlaying || showSentenceTooltip} theme="grey-tooltip" title={clip.sentence.text}>
            <button
              className="play"
              type="button"
              ...
              <span className="padder">{isPlaying ? <StopIcon /> : <PlayOutlineIcon />}</span>
            </button>
          </Tooltip>
          {isPlaying ? (
